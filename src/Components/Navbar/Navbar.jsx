...
import { setSearchTerm } from '../../Features/ProducrSlice'
  ...
  return <header className='bg-white shadow-md'>
    <>
      <div className='py-4 shadow-md'>
        ...
        <nav className='flex justify-between items-center container mx-auto md:py-6 py-8px px-2'>
          ...
          <form className='w-1/2 sm:block hidden'>
            <input type="text" placeholder='Search Product' className='bg-zinc-100 rounded-md border border-zinc-200 focus:outline-none py-3 px-4 w-full' value={searchTerm} onChange={(e)=>dispatch(setSearchTerm(e.target.value))}/>
          </form>
          <div className='relative'>
            <Link to={"/cart"}>
          <ShoppingCart size={54} className='cursor-pointer bg-gray-100 px-3 py-2 rounded-full'/>  
          {itemCount>0 && <span className='absolute -top-2 -right-1 bg-blue-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center'>{itemCount}</span>}
          </Link>
          </div>
        </nav>
    </>
